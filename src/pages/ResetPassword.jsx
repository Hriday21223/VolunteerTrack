...
import { findUserByEmail, updateUser, hashPassword } from '@/api/index.js'
...
export default function ResetPassword() {
  ...
  const onSubmit = (e) => {
    ...
    // The signed-in user's password is updated directly.
    // For other email-based flows, supply the email via state.
    if (user) updateUser(user.id, { passwordHash: hashPassword(pw1) })
    setDone(true)
          ...
          {done ? (
            ...
            <>
              ...
              <p className="text-sm text-earth-500 dark:text-earth-400 mb-6">
                Make it at least 6 characters. Avoid reusing passwords from other accounts.
              </p>
              <form onSubmit={onSubmit} className="space-y-4">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="password" required minLength={6} className="input pl-9" value={pw1} onChange={(e) => setPw1(e.target.value)} />
                  </div>
                </div>
                <div>
                  ...
                  <div className="relative">
                    <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-earth-400" />
                    <input type="password" required minLength={6} className="input pl-9" value={pw2} onChange={(e) => setPw2(e.target.value)} />
                  </div>
                </div>
